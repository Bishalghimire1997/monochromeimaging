...
def weight_generation_test():
     ...
     roi =  cv2.selectROI("Select ROI", image, showCrosshair=True, fromCenter=False)
     print(roi)
     Processing.pixel_correction(roi, [225,255,255],image)
...
def weight_application_test():
     ...
     for i in range(2):
         ...
         wb= Processing.pixel_correction(roib, [255,100,100],image)
        
         wg= Processing.pixel_correction(roig, [100,255,100],image)
         
         wr= Processing.pixel_correction(roir, [100,100,255],image)
         transformation_matrix = np.array([wb[:, 0],wg[:, 1],wr[:, 2 ]]).T
         image_new= image
         transformed_new = Processing.fit_colors(wb,wg,wr,image)
         image = transformed_new
         transformed_odl = Processing.fit(image_new,transformation_matrix)
