...
class ChannelReg():
    ...
    def register_channels(self,image_batch):
        ...
        registered_b = self.__reg(blue,flow_b)
        registered_r = self.__reg(red,flow_r)
        registered_image = [cv2.merge([b,g,r]) for b,g,r in zip(registered_b,green,registered_r)]
        return registered_image
    
        
    def __reg(self,floating_image_list,flow_list):
        registered_images = []
        for image,flow in zip(floating_image_list,flow_list):
            registered_image = self.__fine_reg_using_stn(image,flow)
    ...
    @torch.no_grad()
    def __fine_reg_using_stn(self,floating_image, flow):
    ...
    def warp_batch(self,image_list, flow_list):
        """
        Warp a list of image tensors using corresponding flow fields.
        
        Args:
            image_list: List of Tensors [C, H, W] - source images
            flow_list:  List of Tensors [2, H, W] - flow fields
        
        Returns:
            warped_images: List of Tensors [C, H, W]
        """
        # Stack tensors into a batch
        images = torch.stack([img for img in image_list], dim=0)  # [B, C, H, W]
        flows  = torch.stack([flow for flow in flow_list], dim=0) # [B, 2, H, W]
        ...
        # Compute final warp grid
        warp_grid = base_grid + flow_norm  # [B, H, W, 2]

        # Apply STN warping
        warped = F.grid_sample(images, warp_grid, mode='bilinear', padding_mode='border', align_corners=True)

        # Split back into list of tensors
        warped_images = [warped[i] for i in range(B)]  # Each is [C, H, W]
    ...
    def compute_flow(self, fix_batch, floating_batch):
        ...
        img1_batch = torch.cat(fix_batch, dim=0)  # [B, 3, H, W]
        img2_batch = torch.cat(floating_batch, dim=0)  # [B, 3, H, W]
